...
public class task3 {
	...
	public static void checkArmstrongNumber(int num) {
		...
		int rev = IntStream.iterate(num, i -> i / 10).limit(pow).map(i -> (int) Math.pow(i % 10, pow)).sum();
	...
	public static void checkPalindromeNumber(int num) {
		int copyNum;
		copyNum = num;
		int rev = IntStream.iterate(num, n -> n != 0, i -> i / 10).map(n -> n % 10).reduce(0, (a, b) -> a * 10 + b);
