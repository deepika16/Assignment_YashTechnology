...
public class CityStateStreamTask2 {
	public static void main(String[] args) {
		...
		Stream<City> lessArea = list.stream()
				.sorted(Comparator.comparing(City::getArea_of_city).thenComparing(City::getPopulation));
		lessArea.forEach(System.out::println);

		System.out.println("the city with high pollution_index and high city area");
		Stream<City> higherPopultaion = list.stream()
				.sorted(Comparator.comparing(City::getPollutionIndex).reversed().thenComparing(City::getArea_of_city));
		...
		Stream<City> lowestPollutionIndexlowestArea = list.stream()
		...
		Map<City, Long> counted = list.stream()
